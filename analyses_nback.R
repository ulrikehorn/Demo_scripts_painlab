...
df_rating <- read.csv(paste(path,'/nback_result_',sub,'_ratings.csv', sep = ''), na.strings=c("NA","NaN", " "))

# make a barplot for the hot condition to see the effect on rating
p <- ggplot(df_rating[df_rating$Stim=='hot',], aes(x = Task, y = Rating, fill = Task)) +
...
for (iblock in 1:num_blocks){
  ...
  df$stim[df$block==iblock-1] = df_rating$Stim[iblock]
}
# rename factors
